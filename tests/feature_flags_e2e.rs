...
async fn happy_path_fetch_caches_value_and_matches_request_shape() {
    ...
    let client = faststats_rs::ClientBuilder::new(test_project_name(), test_version(), test_token()).expect("client gets created")
        .feature_flags_factory(|f| f.add_flag(flag))
        .expect("factory configured")
        .build()
        .expect("client builds");

    let flags = client.feature_flags().expect("feature flags enabled by default");
    let value = flags.when_ready(&test_id("checkout_variant")).await.expect("fetch succeeds");
    ...
    let value_again = flags.when_ready(&test_id("checkout_variant")).await.expect("cached hit");
...
async fn ttl_expiry_triggers_a_fresh_fetch() {
    ...
    let flags = client.feature_flags().expect("feature flags enabled by default");
    flags.when_ready(&test_id("short_ttl_flag")).await.expect("first fetch");

    tokio::time::sleep(Duration::from_millis(50)).await;

    // cached() must report expiry, and when_ready must re-fetch.
    assert_eq!(flags.cached(&test_id("short_ttl_flag")), None);
    flags.when_ready(&test_id("short_ttl_flag")).await.expect("second fetch after expiry");
...
async fn opt_in_posts_then_fetches_and_updates_cache() {
    ...
    let flags = client.feature_flags().expect("feature flags enabled by default");
    let value = flags.opt_in(&test_id("beta_feature")).await.expect("opt-in succeeds");
    assert_eq!(value, faststats_rs::FlagValue::from(true));
    assert_eq!(flags.cached(&test_id("beta_feature")), Some(faststats_rs::FlagValue::from(true)));
...
async fn opt_out_posts_then_fetches_and_updates_cache() {
    ...
    let flags = client.feature_flags().expect("feature flags enabled by default");
    let value = flags.opt_out(&test_id("beta_feature_two")).await.expect("opt-out succeeds");
...
async fn concurrent_fetches_for_same_id_share_exactly_one_network_request() {
    ...
    Mock::given(method("POST"))
        .and(path("/v1/check"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"value": "leader-result"})).set_delay(Duration::from_millis(50)))
        .expect(1)
        .mount(&server)
        .await;
    ...
    let flags = std::sync::Arc::clone(client.feature_flags().expect("feature flags enabled by default"));
    ...
    assert_eq!(result_a.expect("task a completes").expect("fetch ok"), faststats_rs::FlagValue::from("leader-result"));
    assert_eq!(result_b.expect("task b completes").expect("fetch ok"), faststats_rs::FlagValue::from("leader-result"));
